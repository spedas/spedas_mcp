"""Optional external data-source adapters for SPEDAS Agent Kit (issues #21, #22).
...
- :mod:`spedas_agent_kit.datasources.hapi` — generic HAPI servers (CDAWeb, PDS-PPI,
  ISWA, LISIRD, university networks) via ``hapiclient`` (issue #21).
- :mod:`spedas_agent_kit.datasources.fdsn` — FDSN/MTH5 magnetotelluric magnetic-field
  stations from EarthScope via ``pyspedas.mth5`` (``mth5`` + ``obspy``)
  (issue #22).

Design contract (mirrors the optional :mod:`spedas_agent_kit.analysis` layer):
