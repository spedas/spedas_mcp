...
8. **Optional render.** If `spedas-mcp[analysis]` / matplotlib is installed, call `render_tplot(input_files=[<separation_refined.csv>], output_file=<bundle>/plots/separation.png, panel_types=["line"], ylog=true)`. If the `[analysis]` extra is not installed, skip plotting and report the CSV path instead. Mark/annotate the conjunction(s) in `notes/`.
