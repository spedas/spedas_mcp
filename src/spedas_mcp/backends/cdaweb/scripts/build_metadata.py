...
def _fetch_from_data_cdf(dataset_id: str, start_date: str, stop_date: str) -> dict | None:
    ...
    """
    from spedas_mcp.backends.cdaweb.metadata import _extract_metadata
    ...
    try:
        from spedas_mcp.backends.cdaweb.fetch import _get_cdf_file_list
    ...
    from spedas_mcp.backends.cdaweb.http import request_with_retry, DOWNLOAD_TIMEOUT
    try:
...
def fetch_one(entry: dict) -> tuple[str, bool, str]:
    ...
    from spedas_mcp.backends.cdaweb.metadata import _fetch_from_master_cdf
