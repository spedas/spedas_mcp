...
def _analysis_dependencies_available() -> bool:
    """Return whether the optional ``spedas-agent-kit[analysis]`` backend is usable.

    The analysis tools are intentionally not registered unless the optional
    backend APIs are present, which keeps a base ``spedas-agent-kit[mcp]`` install from
...
def _spice_resolve_target(name: str) -> dict[str, Any]:
    ...
    """
    from spedas_agent_kit.backends.spice.missions import has_kernels, resolve_mission
...
def _spice_supported_targets_sample(limit: int = 12) -> list[str]:
    ...
    """
    try:
        from spedas_agent_kit.backends.spice import list_supported_missions
...
def _spice_frame_catalog() -> dict[str, Any]:
    ...
    """
    try:
        from spedas_agent_kit.backends.spice import list_frames_with_descriptions
    except Exception:  # pragma: no cover - backend not installed
        return {
    ...
    try:
        from spedas_agent_kit.backends.spice.frames import FRAME_ALIASES
...
def _spice_missing_kernels(mission_keys: list[str]) -> dict[str, Any]:
    ...
    """
    from spedas_agent_kit.backends.spice.kernel_manager import get_kernel_manager
...
def create_server(*, include_analysis_tools: bool | None = None) -> FastMCP:
    ...
    mcp = FastMCP(
        "spedas-agent-kit",
        instructions=(
            "SPEDAS Agent Kit facade for heliophysics workflows. Start with the SPEDAS "
            ...
            "capability_groups": {
                ...
                "compatibility_low_level": {
                    "status": (
                        ...
                        "SPEDAS_AGENT_KIT_COMPAT_TOOLS=1 for existing clients"
                    ),
                    "env_flag": "SPEDAS_AGENT_KIT_COMPAT_TOOLS=1",
                    "prefer": [
                        "browse_data_sources",
                        "load_data_source",
                        "browse_data_parameters",
    ...
    def search_spedas_data_sources(
        ...
        """
        from spedas_agent_kit.workflows import search_data_sources
    ...
    def plan_spedas_observation(
        ...
        """
        from spedas_agent_kit.workflows import plan_observation
    ...
    def compare_cdaweb_pds_spice(science_goal: str = "") -> str:
        """Compare CDAWeb, PDS, and SPICE roles for a SPEDAS Agent Kit science request."""
        from spedas_agent_kit.workflows import compare_sources
    ...
    def create_spedas_analysis_bundle(
        ...
        from spedas_agent_kit.workflows import create_analysis_bundle
    ...
    def browse_observatories() -> str:
        ...
        from spedas_agent_kit.backends.cdaweb.catalog import browse_observatories as _browse_observatories
    ...
    def load_observatory(observatory_id: str) -> str:
        ...
        from spedas_agent_kit.backends.cdaweb.prompts import build_observatory_prompt
    ...
    def browse_parameters(dataset_id: str, dataset_ids: list[str] | None = None) -> str:
        ...
        from spedas_agent_kit.backends.cdaweb.metadata import browse_parameters as _browse_parameters
    ...
    def fetch_data(
        ...
        import pandas as pd
        from spedas_agent_kit.backends.cdaweb.fetch import fetch_data as _fetch_data
    ...
    def browse_pds_missions(query: str | None = None) -> str:
        ...
        from spedas_agent_kit.backends.pds.catalog import browse_missions as _browse_missions
    ...
    def load_pds_mission(mission_id: str) -> str:
        ...
        from spedas_agent_kit.backends.pds.prompts import build_mission_prompt
    ...
    def browse_pds_parameters(dataset_id: str | None = None, dataset_ids: list[str] | None = None) -> str:
        ...
        from spedas_agent_kit.backends.pds.metadata import browse_parameters as _browse_parameters
    ...
    def fetch_pds_data(
        ...
        import pandas as pd
        from spedas_agent_kit.backends.pds.fetch import fetch_data as _fetch_data
    ...
    def get_ephemeris(
        ...
        (issue #29).
        """
        from spedas_agent_kit.backends.spice import get_state, get_trajectory
        from spedas_agent_kit.backends.spice.kernel_manager import get_kernel_manager
    ...
    def compute_distance(
        ...
        """
        import numpy as np
        from spedas_agent_kit.backends.spice import get_trajectory
    ...
    def transform_coordinates(
        ...
        """
        import numpy as np
        from spedas_agent_kit.backends.spice import transform_vector
    ...
    def manage_cdaweb_cache(
        ...
        from spedas_agent_kit.backends.cdaweb.cache import cache_clean, cache_status, rebuild_catalog, refresh_metadata, refresh_time_ranges
    ...
    def manage_pds_cache(
        ...
        from spedas_agent_kit.backends.pds.cache import build_metadata, cache_clean, cache_status, refresh_metadata, refresh_time_ranges, rebuild_catalog
    ...
    def manage_spice_kernels(
        ...
        from spedas_agent_kit.backends.spice.kernel_manager import check_remote_kernels, get_kernel_manager
...
def serve() -> None:
    ...
    parser = argparse.ArgumentParser(description="Unified SPEDAS Agent Kit server")
    ...
    if cdaweb_cache_dir:
        from spedas_agent_kit.backends.cdaweb import configure
    ...
    if pds_cache_dir:
        from spedas_agent_kit.backends.pds.config import configure as configure_pds
